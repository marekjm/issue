    ...
    if os.path.isfile(events_log_path):
        with open(events_log_path) as ifstream:
            try:
                ...
                print(
                    "{}: failed to decode shortlog".format(
...
def write(events_log: typing.List) -> None:
    events_log_size = issue.config.getConfig().get(
        "events_log_size", EVENTS_LOG_SIZE_DEFAULT
    )
...
def append_event(issue_uid: str, event_type: str, parameters: typing.Dict = {}) -> None:
    ...
    if events_log and (
        events_log[-1].get("event") == content.get("event")
        and events_log[-1].get("issue_uid") == "issue_uid"
    ):
        return
...
def append_event_chained_to(
    issue_uid: str, chained_to_these_issues: typing.List
) -> None:
    issue.shortlog.append_event(
        issue_uid=issue_uid,
        event_type=EVENT_TYPE_CHAINED_TO,
        parameters={
...
def _bug_event_without_assigned_weight(event):
    print(
        "{}: {}: event {} does not have a weight assigned".format(
            colorise(COLOR_WARNING, "warning"),
            colorise(COLOR_ERROR, "bug"),
            colorise_repr(COLOR_LABEL, event["event"]),
        )
    )
...
def squash_events_log_aggressive_1(events_log):
    ...
    for event in events_log[1:]:
        if event["issue_uid"] == squashed_events_log[-1]["issue_uid"]:
            last_event_action = EVENTS_LOG_EVENT_WEIGHTS.get(
                squashed_events_log[-1]["event"]
            )
...
def squash_events_log_aggressive_2(events_log):
    ...
    for event in events_log[1:]:
        this_event_action = EVENTS_LOG_EVENT_WEIGHTS.get(event["event"])
        index_of_last_event_for_the_same_issue = rfind_if(
            squashed_events_log, lambda e: e["issue_uid"] == event["issue_uid"]
        )
        if index_of_last_event_for_the_same_issue > -1:
            last_event_action = EVENTS_LOG_EVENT_WEIGHTS.get(
                squashed_events_log[index_of_last_event_for_the_same_issue]["event"]
            )
...
def squash_events_log(events_log, aggressive=0):
    ...
    for event in events_log[1:]:
        if (
            event["issue_uid"] == squashed_events_log[-1]["issue_uid"]
            and event["event"] == squashed_events_log[-1]["event"]
        ):
            continue
        squashed_events_log.append(event)
    if aggressive > 0:
