...
model = {}
with open(filename_ui, "r") as ifstream:
...
try:
    ...
    print(
        "ambiguous shortened command name: '{0}', candidates are: {1}".format(
            name, candidates
        )
    )
...
if "--version" in ui:
    ...
    exit(0)
if (
    clap.helper.HelpRunner(ui=ui, program=sys.argv[0])
    .adjust(options=["-h", "--help"])
    .run()
    .displayed()
):
    exit(0)
...
def expandIssueUID(issue_sha1_part):
    ...
    for i_sha1 in issues:
        if i_sha1.startswith(issue_sha1_part):
            issue_sha1.append(i_sha1)
    if len(issue_sha1) == 0:
...
def expand_issue_uid_or_exir(issue_uid_part):
    try:
        ...
        print(
            "{0}: issue uid {1} did not match anything".format(
                colorise(COLOR_ERROR, "error"),
                colorise_repr(COLOR_HASH, issue_uid_part),
            )
        )
...
def getMessage(template="", fmt={}, ignore="#"):
    ...
    if template and fmt:
        with open(
            os.path.expanduser("~/.local/share/issue/{0}".format(template))
        ) as ifstream:
    ...
    with open(message_path) as ifstream:
        ...
        message = "".join(
            [l for l in message_lines if not l.startswith(ignore)]
        ).strip()
    return message
    ...
    try:
        ...
        for event in events_log:
            ...
            if event_name == "show":
                try:
                    event_description = (
                        issue.util.issues.getIssue(event["issue_uid"])
                        .get("message")
                        .splitlines()[0]
                    )
                ...
                event_description = "sluggified to {}".format(
                    colorise_repr(
                        COLOR_BRANCH_NAME,
            ...
            if event["issue_uid"] in shortened_uids:
                print(
                    "{issue_key} [{event_datetime}] {event_name}{event_description}".format(
                        issue_key=colorise(
                            COLOR_HASH, shortened_uids[event["issue_uid"]]
                        ),
                        event_datetime=colorise(COLOR_DATETIME, event_datetime),
                        event_name=event_name,
                        event_description=(": " * bool(event_description))
                        + event_description,
                    )
                )
            else:
                ...
                print(
                    "{issue_key} [{event_datetime}] {error_msg}: {event_name}{event_description}".format(
...
def _store_release_diff_simple_named_action(release_name, action_name):
    ...
    release_diff_sha1 = "{0}{1}{2}{3}".format(
    ...
    release_diff_file_path = os.path.join(
        get_release_path(release_name), "diff", "{0}.json".format(release_diff_sha1)
    )
...
def commandOpen(ui):
    ...
    if "--parent" in ui:
        try:
            ...
            print(
                '{error}: could not link issue identified by "{parent_uid}":'.format(
                    colorise(COLOR_ERROR, "error"),
    ...
    if original_uid is not None:
        ...
        message_fmt["parent_message"] = formatted_parent_message
    message = (
        getMessage("issue_message", fmt=message_fmt)
        if len(operands) < 1
        else operands[0]
    )
    if not message:
    ...
    if parent_uid is not None:
        try:
            ...
            print(
                '{error}: could not set parent issue (identified by "{parent_uid}"):'.format(
    ...
    if "--chain-to" in ui or "--parent" in ui:
        ...
        for link_issue_sha1 in ui.get("--chain-to") + [
            ...
            try:
                ...
                print(
                    'warning: could not link issue identified by "{0}":'.format(
                        link_issue_sha1
                    ),
                    e,
                )
...
def commandClose(ui):
    ...
    if issue_data["status"] == "closed":
        print(
            "fatal: issue already closed by {0}{1}".format(
                issue_data.get("close.author.name", "Unknown author"),
                (
                    " ({0})".format(issue_data["close.author.email"])
                    if "close.author.email"
                    else ""
                ),
            )
        )
...
def ls_with_details(unique_id, data):
    ...
    issue_open_timestamp = datetime.datetime.fromtimestamp(
        data.get("open.timestamp", 0)
    )
    print(
        "Opened: {when_opened}".format(
    ...
    if data["status"] == "closed":
        ...
        if issue_close_author_email != issue_open_author_email:
            print(
                "Fixer:  {name} <{email}>".format(
                    name=issue_close_author_name,
                    email=issue_close_author_email,
                )
            )
        issue_close_timestamp = datetime.datetime.fromtimestamp(
            data.get("close.timestamp", 0)
        )
        print(
            "Closed: {closed_when}".format(
...
def commandLs(ui):
    ...
    if "--recent" in ui:
        delta_mods_since = (
            issue.config.getConfig().get("default.time.recent", "1day").split(",")
        )
    ...
    for short, i in issues:
        ...
        try:
            ...
            if colored:
                not_indexed_message = (
                    colored.fg("red") + not_indexed_message + colored.attr("reset")
                )
            print("{0} {1}".format(short, not_indexed_message))
            continue

        if "--open" in ui and (
            issue_data["status"] if "status" in issue_data else ""
        ) not in ("open", ""):
            continue
        if (
            "--closed" in ui
            and (issue_data["status"] if "status" in issue_data else "") != "closed"
        ):
            continue
        if (
            "--status" in ui
            and (issue_data["status"] if "status" in issue_data else "")
    ...
    if "--chained-to" in ui:
        chained_issues = issue.util.issues.getIssue(
            expandIssueUID(ui.get("--chained-to"))
        ).get("chained", [])
        issues_to_list = list(
            filter(lambda i: (i[1] in chained_issues), issues_to_list)
        )

    if "--priority" in ui:
        issues_to_list = sorted(
            issues_to_list,
            key=lambda t: int(t[2].get("parameters", {}).get("priority", 1024)),
        )
    ...
    issues_to_list = sorted(
        good_issues_to_list, key=lambda each: each[2]["message"].splitlines()[0]
    )
    ...
    for n, each in enumerate(issues_to_list):
        ...
        if since is not None:
            issue_timestamp = (
                "close.timestamp" if "--closed" in ui else "open.timestamp"
            )
            issue_timestamp = datetime.datetime.fromtimestamp(
                issue_data.get(issue_timestamp, 0)
            )
        ...
        if "--author" in ui:
            ...
            if not (
                author in issue_data["open.author.name"]
                or author in issue_data["open.author.email"]
            ):
                continue
        if ls_keywords:
        ...
        try:
            ...
            if "--details" in ui:
                ...
                if colored:
                    for kw in ls_keywords:
                        first_message_line = first_message_line.replace(
                            kw, (colored.fg("light_green") + kw + colored.attr("reset"))
                        )
                ...
                if "--verbose" in ui or accepted_tags:
                    ...
                    if colored:
                        tags = [
                            (colored.fg("cyan") + t + colored.attr("reset"))
                            for t in tags
                        ]
            ...
            if colored:
                broken_index_message = (
                    colored.fg("red") + broken_index_message + colored.attr("reset")
                )
...
def commandSlug(ui):
    ...
    if slug_format.startswith("@"):
        slug_format = issue.config.getConfig().get(
            "slug.format.{0}".format(slug_format[1:]), ""
        )
    ...
    if "--use-format" in ui:
        slug_format = issue.config.getConfig().get(
            "slug.format.{0}".format(ui.get("--use-format")), ""
        )
        if not slug_format:
    ...
    if "--git-branch" in ui:
        ...
        if (
            allow_branching_from is not None
            and current_branch not in allow_branching_from
        ):
            print(
                "{}: branching from {} not allowed".format(
                    colorise(COLOR_ERROR, "error"),
                    colorise_repr(COLOR_BRANCH_NAME, current_branch),
                )
            )
            print(
                "{}: branching allowed from: {}".format(
                    colorise(COLOR_NOTE, "note"),
                    ", ".join(
                        [
                            colorise_repr(COLOR_BRANCH_NAME, each)
                            for each in allow_branching_from
                        ]
                    ),
                )
            )
...
def commandComment(ui):
    ...
    if "--message" in ui:
        ...
        issue_comment = getMessage(
            "issue_comment_message",
            fmt={
                "issue_sha1": issue_sha1,
                "issue_message": "\n".join(
                    ["#  > {0}".format(l) for l in issue_data["message"].splitlines()]
                ),
            },
        )
    else:
    ...
    issue_comment_sha1 = issue.util.misc.create_hash(
        "{0}{1}{2}".format(issue_sha1, issue_comment_timestamp, issue_comment)
    )
    issue_comment_data = {
    ...
    with open(
        os.path.join(
            issue.util.paths.comments_path_of(issue_sha1),
            "{0}.json".format(issue_comment_sha1),
        ),
        "w",
    ) as ofstream:
...
def commandTag(ui):
    ...
    if subcommand == "ls":
        ...
        if "--missing" in ui:
            ...
            if "--verbose" in ui:
                print(
                    "created {0} tag(s): {1}".format(
                        len(missing_tags), ", ".join(sorted(missing_tags))
                    )
                )
        else:
            try:
...
def commandParam(ui):
    ...
    if "--remove" in ui:
        ...
        issue_differences[0]["params"]["key"] = issue_parameter_key
        issue_differences[0]["params"]["value"] = operands[
            int(not ("--last" in ui)) + 1
        ]
...
def commandShow(ui):
    ...
    if str(ui) == "show":
        ...
        issue_open_author_name = (
            issue_data["open.author.name"]
            if "open.author.name" in issue_data
            else "Unknown Author"
        )
        issue_open_author_email = (
            issue_data["open.author.email"]
            if "open.author.email" in issue_data
            else "Unknown email"
        )
        issue_open_timestamp = (
            datetime.datetime.fromtimestamp(issue_data["open.timestamp"])
            if "open.timestamp" in issue_data
            else "unknown date"
        )
        ...
        if colored:
            opened_by_heading = (
                colored.fg("red") + opened_by_heading + colored.attr("reset")
            )
        print(
            "{}:   {} ({}), on {}".format(
                opened_by_heading,
        ...
        if issue_data["status"] == "closed":
            ...
            if colored:
                closed_by_heading = (
                    colored.fg("green") + closed_by_heading + colored.attr("reset")
                )
            issue_close_author_name = (
                issue_data["close.author.name"]
                if "close.author.name" in issue_data
                else "Unknown Author"
            )
            issue_close_author_email = (
                issue_data["close.author.email"]
                if "close.author.email" in issue_data
                else "Unknown email"
            )
            issue_close_timestamp = (
                datetime.datetime.fromtimestamp(issue_data["close.timestamp"])
                if "close.timestamp" in issue_data
                else "unknown date"
            )
            print(
                "{}:   {} ({}), on {}".format(
        ...
        if project != default_project:
            print(
                "project:     {0} ({1})".format(
                    project, issue_data.get("project.tag", "")
                )
            )
        ...
        if parameters:
            ...
            if colored:
                parameters_heading = (
                    colored.fg("white") + parameters_heading + colored.attr("reset")
                )
            print("\n{}".format(parameters_heading))
            for key in sorted(parameters.keys()):
                value = parameters[key]
                if colored:
                    key = colored.fg("green") + key + colored.attr("reset")
        ...
        if parent_uid:
            ...
            if colored:
                chained_issues_heading = (
                    colored.fg("white") + chained_issues_heading + colored.attr("reset")
                )
        ...
        if "closing_git_commit" in issue_data:
            ...
            if colored:
                closing_git_commit_heading = (
                    colored.fg("white")
                    + closing_git_commit_heading
                    + colored.attr("reset")
                )
                closing_git_commit = (
                    colored.fg("yellow") + closing_git_commit + colored.attr("reset")
                )
            print("\n{}: {}\n".format(closing_git_commit_heading, closing_git_commit))

        issue_comment_thread = dict(
            (issue_data["comments"][key]["timestamp"], key)
            for key in issue_data["comments"]
        )
        if issue_comment_thread and "--comments" in ui:
            ...
            for timestamp in sorted(issue_comment_thread.keys()):
                ...
                indented_message_lines = [
                    "  {}".format(l) for l in issue_comment["message"].splitlines()
                ]
                print("{}".format("\n".join(indented_message_lines)))
                print()
        markLastIssue(issue_sha1)
        ...
        issue_differences = issue.util.issues.getIssueDifferences(
            issue_sha1, *issue.util.issues.listIssueDifferences(issue_sha1)
        )
        ...
        for d in issue_differences_sorted:
            diff_datetime = str(datetime.datetime.fromtimestamp(d["timestamp"])).rsplit(
                ".", 1
            )[0]
            ...
            if diff_action == "open":
                ...
                if (
                    ...
                    message_heading = " with Git commit {0}".format(
                        d["params"]["closing_git_commit"]
                    )
            ...
            # FIXME: this support should be removed after early repositories are converted
                ...
                message_heading = "{} = {}".format(
                    d["params"]["key"], repr(d["params"]["value"])
                )
                ...
                message_heading = "with issue(s) {}".format(
                    ", ".join(d["params"]["sha1"])
                )
            elif diff_action == "chain-unlink":
                action_heading = "unchained"
                message_heading = "from issue(s) {}".format(
                    ", ".join(d["params"]["sha1"])
                )

            if colored:
                diff_datetime_heading = (
                    colored.fg("white") + diff_datetime_heading + colored.attr("reset")
                )
                action_heading = (
                    colored.fg("green") + action_heading + colored.attr("reset")
                )
                author_heading = (
                    colored.fg("white") + author_heading + colored.attr("reset")
                )
                if author_email_heading:
                    author_email_heading = (
                        colored.fg("white")
                        + author_email_heading
                        + colored.attr("reset")
                    )
...
def commandConfig(ui):
    ...
    config_path = os.path.expanduser(
        ("~/.issueconfig.json" if "--global" in ui else "./.issue/config.json")
    )
    ...
    if str(ui) == "get":
        ...
        print(
            (
                json.dumps({config_key: config_value})
                if "--verbose" in ui
                else (config_value if config_value is not None else "null")
            )
        )
    elif str(ui) == "set":
        operands = ui.operands()
        ...
        print(
            (
                ...
                if "--verbose" not in ui
                else json.dumps(config_data, sort_keys=True, indent=2)
            )
        )
...
def commandRemote(ui):
    ...
    if str(ui) == "ls":
        for k, remote_data in remotes.items():
            print(
                ("{}\t{}" if "--verbose" in ui else "{}").format(k, remote_data["url"])
            )
    elif str(ui) == "set":
...
def commandIndex(ui):
    ...
    if "--reverse" in ui:
        for i in issue.util.issues.ls():
            if not os.listdir(
                os.path.join(issue.util.paths.issues_path(), i[:2], i, "diff")
            ):
                issue_list.append(i)
...
def commandStatistics(ui):
    ...
    if True:
        ...
        for i in range(limit):
            print(
                (colored.fg("green") + "#" + colored.attr("reset") if colored else "#"),
                end="",
            )
        for i in range(N - limit):
            print(
                (colored.fg("red") + "#" + colored.attr("reset") if colored else " "),
                end="",
            )
        ...
        if colored:
            closed_issues_count_heading = (
                colored.fg("green")
                + closed_issues_count_heading
                + colored.attr("reset")
            )
            open_issues_count_heading = (
                colored.fg("red") + open_issues_count_heading + colored.attr("reset")
            )
            issues_count_heading = (
                colored.fg("cyan") + issues_count_heading + colored.attr("reset")
            )
            percentage_closed_heading = (
                colored.fg("white") + percentage_closed_heading + colored.attr("reset")
            )
        print(
            "| {closed}/{still_open}/{total} ({perc_closed} closed)".format(
    ...
    if True:
        if closed_issues_count:
            ...
            lifetimes_of_closed = list(
                map(lambda each: (each[0] - each[1]), open_closed_timestamps)
            )
            lifetimes_of_still_open = list(
                map(lambda each: (each[0] - each[1]), still_open_timestamps)
            )
            ...
            total_lifetime_of_all = sum_or_none_if_empty(
                lifetimes_of_closed + lifetimes_of_still_open
            )
            ...
            avg_lifetime_of_all = avg_or_none(
                lifetimes_of_closed + lifetimes_of_still_open
            )
            med_lifetime_of_all = median_or_none(
                lifetimes_of_closed + lifetimes_of_still_open
            )

            if (total_lifetime_of_closed is None) or (
                total_lifetime_of_still_open is None
            ):
                ...
                perc_total_closed_of_open = (
                    total_lifetime_of_closed / total_lifetime_of_still_open
                ) * 100
                perc_total_closed_of_all = (
                    total_lifetime_of_closed / total_lifetime_of_all
                ) * 100
                perc_avg_closed_of_open = (
                    avg_lifetime_of_closed / avg_lifetime_of_still_open
                ) * 100
                perc_avg_closed_of_all = (
                    avg_lifetime_of_closed / avg_lifetime_of_all
                ) * 100

                perc_total_open_more_than_closed = (
                    (total_lifetime_of_still_open / total_lifetime_of_closed) * 100
                ) - 100
            ...
            print(
                "lifetime of {} {} issues:".format(
                    len(closed_issues),
                    colorise("green", "closed"),
                )
            )
            print(
                "    total: {} ({}% of open, {}% of all)".format(
...
def commandReleaseOpen(ui):
    ...
    if current_next_release:
        print(
            "error: a release is currently opened: {}".format(
                repr(current_next_release)
            )
        )
        print("note: only one release can be opened at a time")
        print(
            "note: close release {} before opening new one".format(current_next_release)
        )
        exit(1)
...
def commandReleaseClose(ui):
    ...
    if release_name != current_next_release:
        ...
        if current_next_release:
            print(
                "note: {} is the currently opened release".format(current_next_release)
            )
        else:
    ...
    if not release_notes:
        release_notes = getMessage(
            "release_notes_message", fmt={"release_name": release_name}, ignore="%%"
        )
    if not release_notes:
...
def commandReleaseLs(ui):
    ...
    print(
        "\n".join(
            sorted(os.listdir(os.path.join(issue.util.paths.releases_path(), "r")))
        )
    )
...
def commandLog(ui):
    ...
    if str(ui) == "squash":
        ...
        if initial_size < 2:
            print(
                "{}: events_log too short to shorten".format(
                    colorise(COLOR_WARNING, "warning")
                )
            )
            return
        ...
        if final_size < initial_size:
            print(
                "{}: shortened events_log from {} to {} entries".format(
                    colorise(COLOR_NOTE, "note"), initial_size, final_size
                )
            )
...
def dispatch(ui, *commands, overrides={}, default_command=""):
    ...
    try:
        if ui_command in overrides:
            ...
            ui_command = "command" + "".join(
                [(s[0].upper() + s[1:]) for s in ui_command.split("-")]
            )
            for cmd in commands:
        ...
        print(
            "{}: not an issue repository (or any of the parent directories): {}".format(
                colorise(COLOR_ERROR, "error"),
...
dispatch(
    ui,  # first: pass the UI object to dispatch
